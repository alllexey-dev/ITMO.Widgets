...
class SportScheduleRepositoryImpl @Inject constructor(
    ...
    private val combined = combine(
        ...
        schedulesState.throwableOrNull()?.let {
        ...
        val errors = listOfNotNull(
            queueEntries.throwableOrNull(),
            queues.throwableOrNull(),
            friendsBookings.throwableOrNull()
        )
        ...
        try {
            val result = withContext(Dispatchers.IO) {
                ...
                response.body()?.result?.associate {
                    it.date to it.lessons.orEmpty().map { l -> l.toModel() }
                }
            }
            ...
            scheduleFlow.emit(DataState.Error(e))
        }
    }
    ...
    override suspend fun refreshSportFilters() {
        try {
            ...
            filtersFlow.emit(DataState.Error(e))
        }
    }
    ...
    override suspend fun refreshSportTimeSlots() {
        try {
            ...
            timeSlotsFlow.emit(DataState.Error(e))
        }
    }
}
