...
- The meeting link is shown by host (`bbb.itmo.ru`, not "Zoom"), the button is
  `Открыть ссылку` with the Material `videocam` icon, and schedule cards mark
  linked lessons with the same icon. Pending sport rows open their own sheet with the status
